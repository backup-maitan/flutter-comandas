...
import 'package:comandas/frontend/telasBalcao/cardapioBalcao.dart';
import 'package:comandas/frontend/widgets/cardPedido.dart';
import 'package:flutter/material.dart';
import 'package:scoped_model/scoped_model.dart';

// ignore: must_be_immutable
class PedidosBalcao extends StatefulWidget {

   String cliente;
   PedidosBalcao(this.cliente);

  @override
  _PedidosBalcaoState createState() => _PedidosBalcaoState();
}

class _PedidosBalcaoState extends State<PedidosBalcao> {
  @override
  Widget build(BuildContext context) {
    return ScopedModelDescendant<UsuarioModel>(
      builder: (context, child, model){
        return Scaffold(
          appBar: AppBar(
            title: Text(widget.cliente + ' - Balcão'),
          ),
          body: StreamBuilder(
            ...
            builder: (context, snapshot){
              if(!snapshot.hasData){
                ...
                for(DocumentSnapshot documentSnapshot in querySnapshot.docs){
                  if(documentSnapshot.data()['origem'] == 'balcao' && documentSnapshot.data()['cliente'] == widget.cliente && documentSnapshot.data()['situacao'] != 'cancelado'){
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: (){
           Navigator.of(context).push(MaterialPageRoute(builder: (context)=> CardapioBancao(widget.cliente)));
          },
        ),
        );
      }
